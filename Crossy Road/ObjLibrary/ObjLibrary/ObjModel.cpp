...
bool ObjModel :: isPathPropagation ()
{
	return ms_is_path_propagation;
}

void ObjModel :: setPathPropagation (bool propagate)
{
	ms_is_path_propagation = propagate;
}
...
bool ObjModel :: ms_is_path_propagation = true;



ObjModel :: ObjModel ()
		: m_material_libraries(),
		  m_vertices(),
...
unsigned int ObjModel :: getPointSetVertexCount (unsigned int point_set) const
{
	...
	return m_point_sets[point_set].mv_vertices.size();
}
...
unsigned int ObjModel :: getPointSetVertexIndex (unsigned int point_set, unsigned int vertex) const
{
	...
	return m_point_sets[point_set].mv_vertices[vertex];
}
...
unsigned int ObjModel :: getLineVertexCount (unsigned int line) const
{
	...
	return m_lines[line].mv_vertices.size();
}
...
unsigned int ObjModel :: getLineVertexIndex (unsigned int line, unsigned int vertex) const
{
	...
	return m_lines[line].mv_vertices[vertex].m_vertex;
}
...
unsigned int ObjModel :: getLineVertexTextureCoordinates (unsigned int line, unsigned int vertex) const
{
	...
	return m_lines[line].mv_vertices[vertex].m_texture_coordinates;
}
...
unsigned int ObjModel :: getFaceCount (unsigned int face_set) const
{
	...
	return m_face_sets[face_set].mv_faces.size();
}
...
unsigned int ObjModel :: getFaceVertexCount (unsigned int face_set, unsigned int face) const
{
	...
	return m_face_sets[face_set].mv_faces[face].mv_vertices.size();
}
...
unsigned int ObjModel :: getFaceVertexIndex (unsigned int face_set, unsigned int face, unsigned int vertex) const
{
	...
	return m_face_sets[face_set].mv_faces[face].mv_vertices[vertex].m_vertex;
}
...
unsigned int ObjModel :: getFaceVertexTextureCoordinates (unsigned int face_set, unsigned int face, unsigned int vertex) const
{
	...
	return m_face_sets[face_set].mv_faces[face].mv_vertices[vertex].m_texture_coordinates;
}
...
unsigned int ObjModel :: getFaceVertexNormal (unsigned int face_set, unsigned int face, unsigned int vertex) const
{
	...
	return m_face_sets[face_set].mv_faces[face].mv_vertices[vertex].m_normal;
}
...
unsigned int ObjModel :: getFaceCountTotal () const
{
	...
	for(unsigned int i = 0; i < m_face_sets.size(); i++)
		total += m_face_sets[i].mv_faces.size();
	return total;
}
...
{
	if(m_valid)
		cout << m_file_path << m_file_name << " (valid)" << endl;
	else
		cout << m_file_path << m_file_name << " (invalid)" << endl;
...
{
	cout << "Libraries referenced by \"" << m_file_path << m_file_name << "\":" << endl;
	for(unsigned int m = 0; m < m_material_libraries.size(); m++)
		cout << "\t\"" << m_material_libraries[m].m_name << "\"" << endl;
}
...
void ObjModel :: printBadMaterials () const
{
	for(unsigned int s = 0; s < getFaceSetCount(); s++)
		...
		{
			cout << "Invalid material referenced in \"" << m_file_path << m_file_name << "\": "
			     << "\"" << m_face_sets[s].m_material_name << "\"" << endl;
		}
}
	...
	for(unsigned int s = 0; s < getFaceSetCount(); s++)
		for(unsigned int f = 0; f < getFaceCount(s); f++)
		{
			glBegin(GL_LINE_LOOP);
				...
				{
					unsigned int vertex = m_face_sets[s].mv_faces[f].mv_vertices[v].m_vertex;
	...
	glBegin(GL_LINES);
		for(unsigned int s = 0; s < getFaceSetCount(); s++)
			for(unsigned int f = 0; f < getFaceCount(s); f++)
				...
				{
					unsigned int vertex = m_face_sets[s].mv_faces[f].mv_vertices[v].m_vertex;
					unsigned int normal = m_face_sets[s].mv_faces[f].mv_vertices[v].m_normal;
...
void ObjModel ::  save (const string& filename, ostream& r_logstream) const
{
	...
	if(m_point_sets.size() > 0)
	{
		...
		{
			output_file << "p";
			for(unsigned int i = 0; i < m_point_sets[p].mv_vertices.size(); i++)
				output_file << " " << (m_point_sets[p].mv_vertices[i] + 1);
			output_file << endl;
		}
		output_file << endl;
		output_file << endl;
	...
	if(m_lines.size() > 0)
	{
		...
		{
			output_file << "l";
			for(unsigned int i = 0; i < m_lines[l].mv_vertices.size(); i++)
			{
				output_file << " " << (m_lines[l].mv_vertices[i].m_vertex + 1);

				if(m_lines[l].mv_vertices[i].m_texture_coordinates != NO_TEXTURE_COORDINATES)
					output_file << "/" << (m_lines[l].mv_vertices[i].m_texture_coordinates + 1);
			}
			output_file << endl;
		}
		output_file << endl;
	...
	if(m_face_sets.size() > 0)
	{
		...
		for(unsigned int s = 0; s < m_face_sets.size(); s++)
		{
			...
			for(unsigned int f = 0; f < m_face_sets[s].mv_faces.size(); f++)
			{
				output_file << "f";
				for(unsigned int i = 0; i < m_face_sets[s].mv_faces[f].mv_vertices.size(); i++)
				{
					output_file << " " << (m_face_sets[s].mv_faces[f].mv_vertices[i].m_vertex + 1);

					if(m_face_sets[s].mv_faces[f].mv_vertices[i].m_texture_coordinates != NO_TEXTURE_COORDINATES)
					{
						output_file << "/" << (m_face_sets[s].mv_faces[f].mv_vertices[i].m_texture_coordinates + 1);

						if(m_face_sets[s].mv_faces[f].mv_vertices[i].m_normal != NO_NORMAL)
							output_file << "/" << (m_face_sets[s].mv_faces[f].mv_vertices[i].m_normal + 1);
					}
					else if(m_face_sets[s].mv_faces[f].mv_vertices[i].m_normal != NO_NORMAL)
						output_file << "//" << (m_face_sets[s].mv_faces[f].mv_vertices[i].m_normal + 1);
...
void ObjModel :: setPointSetVertexIndex (unsigned int point_set, unsigned int vertex, unsigned int index)
{
	...
	m_point_sets[point_set].mv_vertices[vertex] = index;
...
void ObjModel :: setLineVertexIndex (unsigned int line, unsigned int vertex, unsigned int index)
{
	...
	m_lines[line].mv_vertices[vertex].m_vertex = index;
	if(index >= getVertexCount())
		m_valid = false;
...
void ObjModel :: setLineVertexTextureCoordinates (unsigned int line, unsigned int vertex, unsigned int index)
{
	...
	m_lines[line].mv_vertices[vertex].m_texture_coordinates = index;
...
void ObjModel :: setFaceVertexIndex (unsigned int face_set, unsigned int face, unsigned int vertex, unsigned int index)
{
	...
	m_face_sets[face_set].mv_faces[face].mv_vertices[vertex].m_vertex = index;
	if(index >= getVertexCount())
		m_valid = false;
...
void ObjModel :: setFaceVertexTextureCoordinates (unsigned int face_set, unsigned int face, unsigned int vertex, unsigned int index)
{
	...
	m_face_sets[face_set].mv_faces[face].mv_vertices[vertex].m_texture_coordinates = index;
	if(index >= getTextureCoordinateCount() && index != NO_TEXTURE_COORDINATES)
		m_valid = false;
...
void ObjModel :: setFaceVertexNormal (unsigned int face_set, unsigned int face, unsigned int vertex, unsigned int index)
{
	...
	m_face_sets[face_set].mv_faces[face].mv_vertices[vertex].m_normal = index;
...
void ObjModel :: addMaterialLibrary (const string& library, ostream& r_logstream)
{
	...
	if(ms_is_path_propagation)
		m_material_libraries.push_back(MaterialLibrary(m_file_path, library, r_logstream));
	else
...
unsigned int ObjModel :: addPointSetVertex (unsigned int point_set, unsigned int vertex)
{
	...
	assert(vertex >= 0);
	unsigned int id = m_point_sets[point_set].mv_vertices.size();
	m_point_sets[point_set].mv_vertices.push_back(vertex);
...
unsigned int ObjModel :: addLineVertex (unsigned int line, unsigned int vertex, unsigned int texture_coordinates)
{
	...
	unsigned int id = m_lines[line].mv_vertices.size();
	m_lines[line].mv_vertices.push_back(LineVertex(vertex, texture_coordinates));
...
unsigned int ObjModel :: addFace (unsigned int face_set)
{
	...
	unsigned int id = m_face_sets[face_set].mv_faces.size();
	m_face_sets[face_set].mv_faces.push_back(Face());
	m_valid = false;
...
unsigned int ObjModel :: addFaceVertex (unsigned int face_set, unsigned int face, unsigned int vertex, unsigned int texture_coordinates, unsigned int normal)
{
	...
	unsigned int id = m_face_sets[face_set].mv_faces[face].mv_vertices.size();
	m_face_sets[face_set].mv_faces[face].mv_vertices.push_back(FaceVertex(vertex, texture_coordinates, normal));
...
void ObjModel :: validate ()
{
	...
	for(unsigned int p = 0; p < m_point_sets.size(); p++)
	{
		if(m_point_sets[p].mv_vertices.size() < 1)
		{
			m_valid = false;
		...
		for(unsigned int v = 0; v < m_point_sets[p].mv_vertices.size(); v++)
		{
			unsigned int vertex = m_point_sets[p].mv_vertices[v];
	...
	for(unsigned int l = 0; l < m_lines.size(); l++)
	{
		if(m_lines[l].mv_vertices.size() < 2)
		{
			m_valid = false;
		...
		for(unsigned int v = 0; v < m_lines[l].mv_vertices.size(); v++)
		{
			unsigned int vertex = m_lines[l].mv_vertices[v].m_vertex;
			unsigned int texture_coordinates = m_lines[l].mv_vertices[v].m_texture_coordinates;
...
*/
		...
		for(unsigned int f = 0; f < m_face_sets[s].mv_faces.size(); f++)
		{
			unsigned int face_vertex_count = m_face_sets[s].mv_faces[f].mv_vertices.size();
			...
			{
				unsigned int vertex              = m_face_sets[s].mv_faces[f].mv_vertices[v].m_vertex;
				unsigned int texture_coordinates = m_face_sets[s].mv_faces[f].mv_vertices[v].m_texture_coordinates;
				unsigned int normal              = m_face_sets[s].mv_faces[f].mv_vertices[v].m_normal;
...
void ObjModel :: drawFaceSet (unsigned int face_set) const
{
	...
	for(unsigned int f = 0; f < getFaceCount(face_set); f++)
	{
		...
		{
			unsigned int vertex              = m_face_sets[face_set].mv_faces[f].mv_vertices[v].m_vertex;
			unsigned int texture_coordinates = m_face_sets[face_set].mv_faces[f].mv_vertices[v].m_texture_coordinates;
			unsigned int normal              = m_face_sets[face_set].mv_faces[f].mv_vertices[v].m_normal;
...
void ObjModel :: removeLastFace (unsigned int face_set)
{
	...
	m_face_sets[face_set].mv_faces.pop_back();
	m_valid = false;
}
...
ObjModel :: FaceSet :: FaceSet () : mv_faces()
{
	m_material_name = "";
	mp_material     = NULL;
...
ObjModel :: FaceSet :: FaceSet (const string& material_name, Material* p_material) : mv_faces()
{
...
ObjModel :: FaceSet :: FaceSet (const ObjModel :: FaceSet& original) : mv_faces(original.mv_faces)
{
