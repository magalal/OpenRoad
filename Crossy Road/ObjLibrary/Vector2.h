...
//
	...
	Vector2 componentRatio (const Vector2& other) const
	{
		assert(isFinite());
		assert(other.isFinite());
...
//
	...
	Vector2 componentRatioSafe (const Vector2& other) const
	{
		assert(isFinite());
		assert(other.isFinite());
...
//           the specified Vector2.  Orthogonal is exactly the
...
//
	...
	bool isOrthogonal (const Vector2& other) const
	{
		...
		if(fabs(getCosAngle(other)) < 1e-6)
			return true;
		else
			return false;
	}
