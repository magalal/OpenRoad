...
void Material :: loadDisplayTextures (const std::string& path)
{
	...
	if(mp_diffuse_map == NULL && m_diffuse_filename.compare("") != 0)
		mp_diffuse_map = &(TextureManager::get(path + m_diffuse_filename));
	...
	if(mp_ambient_map == NULL && m_ambient_filename.compare("") != 0)
		mp_ambient_map = &(TextureManager::get(path + m_ambient_filename));
	...
	if(mp_specular_map == NULL && m_specular_filename.compare("") != 0)
		mp_specular_map = &(TextureManager::get(path + m_specular_filename));
...
void Material :: loadAllTextures (const std::string& path)
{
	if(mp_ambient_map == NULL && m_ambient_filename.compare("") == 0)
		mp_ambient_map = &(TextureManager::get(path + m_ambient_filename));
	...
	if(mp_specular_exponent_map == NULL && m_specular_exponent_filename.compare("") == 0)
		mp_specular_exponent_map = &(TextureManager::get(path + m_specular_exponent_filename));

	if(mp_transparency_map == NULL && m_transparency_filename.compare("") == 0)
		mp_transparency_map = &(TextureManager::get(path + m_transparency_filename));

	if(mp_decal_map == NULL && m_decal_filename.compare("") == 0)
		mp_decal_map = &(TextureManager::get(path + m_decal_filename));

	if(mp_displacement_map == NULL && m_displacement_filename.compare("") == 0)
		mp_displacement_map = &(TextureManager::get(path + m_displacement_filename));

	if(mp_bump_map == NULL && m_bump_filename.compare("") == 0)
		mp_bump_map = &(TextureManager::get(path + m_bump_filename));
}
