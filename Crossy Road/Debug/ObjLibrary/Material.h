...
//  Material
//
//  Only a single texture is displayed at a time, even if
//    multiple are specified for a material.  This is due to the
//    limitations of fixed-pipeline OpenGL.  The single texture
//    applies to all of ambient, diffuse, and specular.  The
//    texture to use is chosen as follows:
...
//           displaying this Material.
//  Parameter(s):
//    <1> path: The file path to prepend
//  Precondition(s): N/A
//  Returns: N/A
//  Side Effect: All textures that would be needed to display
//               this Material are loaded.  Path is prepended to
//               each texture name as the texture is loaded.
//

	void loadDisplayTextures (const std::string& path);
...
//  Returns: N/A
//  Side Effect: All textures referenced by this Material are
//               loaded.  path is prepended to each texture name
